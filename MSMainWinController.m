...
- (void) executeForImage:(NSImage*) startImage
{
	if([baseImageView image] != nil)
	{
		...
		CIImage *baseCIImage =[CIImage imageWithData:[startImage TIFFRepresentation]];
		...
		NSString *infoText = [NSString stringWithFormat:@"%ix%i -(x%f)-> %ix%i", (int)[baseCIImage extent].size.width,  (int)[baseCIImage extent].size.height, scaleFactor, (int)[scaledImage extent].size.width, (int)[scaledImage extent].size.height];		
